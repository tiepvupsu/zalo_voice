...
def merge_wavs(fns, new_fn = 'mearged.wav'):
    """
    merge wav files saved in fns
    INPUT:
        fns: list of file paths
    """
    data = []
    for fn in fns: 
        data_, sr = torchaudio.load(fn)
    data.append(data_)

    a = torch.cat(data, dim = 0)
    torchaudio.save(new_fn, a, sr)

def _fn_clustering_merge(fns):
    fns.sort()
    groups = []
    for i, fn in enumerate(fns):
        fn_no_ext = fn.split('/')[-1].split('.')[0].strip('_').split('_') # avoid _ at the end
...
def merge_voices(csv_file):
    gender = ['female', 'male']
    accent = ['north', 'central', 'south']
    cates = [g + '_' + a for g in gender for a in accent]
    _fns = defaultdict(list)
    with open(csv_file) as fp:
        cnt = 0
        for line in fp:
            fn, lb = line.strip().split(',')
            _fns[lb].append(BASE_DIR + lb + '/' + fn + '.wav')
    fns = [] # fns[i] is a list of all filenames in class i  
    for cate in cates:
        sub_dir = os.path.join(cf.BASE_DIR_MERGED, cate)
        if not os.path.exists(sub_dir):
            os.makedirs(sub_dir)
        groups = _fn_clustering(_fns[cate])
        for i, group in enumerate(groups):
            # pdb.set_trace()
            new_fn = cf.BASE_DIR_MERGED + '/'.join(group[0].split('/')[-2:])
            merge_wavs(group, new_fn)
            if (i + 1)%100 == 0:
                print('finished {}/{}'.format(i+1, len(groups)))

def _train_test_split_1(voice_fns, test_size = 0.1):
    total_files = sum(map(len, voice_fns))
    n_tests = int(test_size*total_files)
    # mix 
    idx = np.random.permutation(len(voice_fns))
    train_fns = []
    test_fns = [] 
    acc_sum = 0 # accumulated sum so far 
    test_flag = True
    for i in list(idx):
        fns = voice_fns[i]
        if test_flag:            
            test_fns.extend(fns)
            acc_sum += len(fns)
            if acc_sum >= n_tests: test_flag = False  
        else: 
            train_fns.extend(fns)
    return train_fns, test_fns 

def mytrain_test_split_voice(base_, cates, test_size = 0.1, random_state = None):
    """
    My own data split for the Voice challenge
    INPUT:
        fns: a list of filename lists 
        test_size: a float number between 0.0 and 1.0
            proportion of test size
        random_state: None or int 
            seed for numpy
    OUTPUT:
        train_fns: list of strings 
            list of all training filenames
        test_fns: list of strings 
            list of all test filenamse
        train_lbs: list of integers
            list of all training labels
        test_fns: list of integers 
...
#     accent = ['north', 'central', 'south']
    ...
    with open(csv_file) as fp:
        for line in fp:
            fn, lb = line.strip().split(',')
            _fns[lb].append(base_ + lb + '/' + fn + '.wav')
    fns = [] # fns[i] is a list of all filenames in class i  
    for cate in cates:
        fns.append(_fns[cate])

    # for i, cate in enumerate(cates):
        # fns.append([base_ + cates[i] + '/' + fn for fn in os.listdir(base_ + cate)])    
    pdb.set_trace()
    train_fns = []
    test_fns = []
    train_lbs = []
    ...
    for i, fn_class in enumerate(fns):        
        ...
        _train_fns, _test_fns = _train_test_split_1(_groups, test_size = test_size)
        train_fns.extend(_train_fns)        
        test_fns.extend(_test_fns)
        train_lbs.extend([i]*len(_train_fns))
        test_lbs.extend([i]*len(_test_fns))
    
    return train_fns, test_fns, train_lbs, test_lbs, None
    
def _train_test_split_2(name_clusters, test_size = 0.1, offset = 0):
    total_clusters = len(name_clusters)
#     total_files = sum(map(len, name_clusters))
    n_test_clusters = int(test_size*total_clusters) # split by voice
    # mix 
    idx = np.random.permutation(total_clusters)
    train_fns = []
    test_fns = [] 
    nested_train_ids = []
    acc_sum = 0 # accumulated sum so far 
    ...
    for i in idx:
        fns = name_clusters[i]
        if i < n_test_clusters:            
            test_fns.extend(fns)
#             acc_sum += len(fns)
#             if acc_sum >= n_tests: test_flag = False  
        else: 
            train_fns.extend(fns)
            nested_train_ids.append([offset + j for j in range(len(fns))])
            offset += len(fns)
    return train_fns, test_fns, nested_train_ids 

def mytrain_test_split_voice2(base_, cates, test_size = 0.1, 
                              random_state = None):
    """
    My own data split for the Voice challenge
    INPUT:
    ...
    OUTPUT:
        ...
        test_fns: list of integers 
            list of all test labels
        nested_ids: list of lists of lists
...
#     pdb.set_trace()
    ...
    for i, fn_class in enumerate(fns):  
        _groups = _fn_clustering(fn_class)
        print(len(fn_class), len(_groups))
        _train_fns, _test_fns, _nested_train_ids = _train_test_split_2(_groups, test_size = test_size, offset = offset)
        train_fns.extend(_train_fns)        
        test_fns.extend(_test_fns)
        train_lbs.extend([i]*len(_train_fns))
        test_lbs.extend([i]*len(_test_fns))
        nested_train_ids.append(_nested_train_ids)
        offset += len(_train_fns)
    
    return train_fns, test_fns, train_lbs, test_lbs, nested_train_ids
